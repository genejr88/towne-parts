...
- **Tier ≠ Stealth's own package selector, don't confuse the two**: Stealth's product page has its own "Rack Only" / "Rack + Tow Combo" toggle — this is unrelated to Towne's 3 install-fee tiers. `STEALTH_PACKAGE_FOR_TIER` in `HitchQuotes.jsx` maps our tier to which of Stealth's 2 packages to actually select when placing the order (RACK_ONLY→Rack Only, RACK_AND_TOW/RACK_TOW_WIRING→Rack + Tow Combo) — shown on the tier picker, the saved-quote confirmation, and the printed quote so whoever orders from Stealth never has to guess
- **IMPORTANT — cached `HitchKit.price` is the Rack Only base price ONLY, not the tow combo price.** Confirmed by inspecting the live site: Stealth's "Rack + Tow Combo" option isn't a real Shopify variant, it's the BOLD Product Options app bundling in a *second, separate* "Towing Conversion Kit" product and summing the prices client-side (e.g. Volvo V60: $638 base + $269 conversion kit = $907 combo). That surcharge is computed by JS that only runs in a real browser — it is NOT in the public `products.json` feed, and a raw HTTP fetch of the product page (no JS execution) doesn't contain it either, so **do not attempt to auto-scrape or re-derive the combo price server-side** — it would require a headless-browser dependency to chase a number that can silently drift if Stealth changes their app config, which is worse than not having it. Instead: the "Hitch Kit" price field on the New Quote tab is a plain editable input (defaults to the cached base price, sent as `kitPriceOverride` in `POST /api/hitches/quotes`), and an amber warning + "Check live price" link appears whenever a tow tier is selected, prompting staff to verify/correct it against the live page before saving.
